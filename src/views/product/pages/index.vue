<template>
    ...
    <div class="p-content-container">
        ...
        <van-cell v-for="item in factors" :key="item.key" :class="{'fl':(item.readonly === true ||item.readonly === false),'w100':item.type === 'label'}" @click="chooseRefuel(item,item.readonly)">
            <template #title>
                <div class="custom-title" :class="[refuelArr.includes(item.key) ? 'bg-color' : '',item.readonly === true ? 'fly': '']">{{item.title}}</div>
            </template>
            <template #default>
                <van-tag :class="[ currentActivity[item.key] === item1.code  ? 'active' : '',item.key === 'insure' ? 'fly': '']" v-for="(item1) in item.list" :key="item1.code" :title="currentActivity[item.key]" size="large" @click.native="changeCurrent(item.key,item1.code) ">{{item1.text}}</van-tag>
            </template>
        ...
        <div class="terms">
            ...
            <ul class="terms_title">
                <li>
                    <a target="_blank" href="###">术后家庭护理服务介绍</a>
                </li>
                <li>
                    <a target="_blank" href="###">医疗垫付服务介绍</a>
                </li>
                <li>
                    <a target="_blank" href="###">肿瘤特药服务介绍</a>
                </li>
                <li>
                    <a target="_blank" href="###">重疾绿通服务介绍</a>
                </li>
            </ul>
        </div>
        ...
        <van-tabs v-model="activeName" scrollspy sticky>
            <van-tab v-for="(item,index) in detailInfo.introduceChartDetails" :title="item.pageName" :key="item.pageName" name="item.pageName">
                <!-- <div v-for="(item,index) in detailInfo.introduceChartDetails" :key="item.pageName"> -->
                    <div class="item" v-for="item1 in item.list" :key="item1.id">
                        <img style="width: 100%" v-if="item1.introduceImg1" :src="item1.introduceImg1" />
                        <img style="width: 100%" v-if="item1.introduceImg2" :src="item1.introduceImg2" />
                        <img style="width: 100%" v-if="item1.introduceImg3" :src="item1.introduceImg3" />
                    </div>
                <!-- </div> -->
            </van-tab>
        </van-tabs>

        <!-- <div class="m-product-info-content">
    ...
    async getProductDetail() {
      ...
      if (resultFlag) {
        this.detailInfo = Object.assign({}, resultContent);
         this.detailInfo.introduceChartDetails.sort(function(a, b) {
...
.m-product-logo {
  ...
  div {
    width: 90px;
    height: 16px;
    background: url("~@/assets/image/common/logo03.png") no-repeat left 60%;
